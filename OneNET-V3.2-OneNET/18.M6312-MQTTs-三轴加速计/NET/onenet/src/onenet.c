...
extern unsigned char m6312_buf[512];
...
_Bool OneNET_RegisterDevice(void)
{
	...
	while(M6312_SendCmd("AT+IPSTART=\"TCP\",\"183.230.40.33\",80\r\n", "CONNECT"))
		DelayXms(500);
	...
	M6312_SendData((unsigned char *)send_ptr, strlen(send_ptr));
	...
	*/
	...
	data_ptr = (char *)M6312_GetIPD(250);							//等待平台响应
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Xg\":[{\"v\":%f}],", adxl362Info.x);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Yg\":[{\"v\":%f}],", adxl362Info.y);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Zg\":[{\"v\":%f}]", adxl362Info.z);
	strcat(buf, text);
