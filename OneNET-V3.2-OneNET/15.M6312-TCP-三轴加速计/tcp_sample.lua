	...
	if t_len >= 5 then
		if dev:byte(1) == 0xAA and dev:byte(2) == 0x55 then
			...
			if dev:byte(t_len - 1) == 0x55 and dev:byte(t_len) == 0xAA then
				...
				val_num = DWORD(dev:bytes(4,4))
				add_val(j,"x",t,val_num / 100)
				val_num = DWORD(dev:bytes(8,4))
				add_val(j,"y",t,val_num / 100)
				val_num = DWORD(dev:bytes(12,4))
				add_val(j,"z",t,val_num / 100)
